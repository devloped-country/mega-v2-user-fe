...
<<<<<<< Updated upstream
      ...
      />
=======
          return <NoteItem key={id} title={title} desc={content} date={time} isSelected={selectedIds.includes(id)} isRead={true} onClick={() => handleClickList(id)} />;
        })}
      </ul>
      {isShowingModal && <NoteModal handleClose={handleClose} id={id} />}
      <img src={`${import.meta.env.VITE_CLOUD_FRONT_ID}/Frame 565.svg`} onClick={handleDeleteSelected} alt="메일 삭제" className={styles.button} />
>>>>>>> Stashed changes
    </section>
  );
}
