...
function PasswordList({ onAction }) {
  ...
  return (
    <div className={styles.wrapper}>
      <InputPassword title="기존 비밀번호" value={password} setEditPassword={setPassword} />
      {isShowingPWError && <span className={styles.validateMessage}>비밀번호가 틀렸어요.</span>}
      <InputPassword title="새 비밀번호" value={passwordInfo.editpassword} setEditPassword={(value) => setPasswordInfo((prev) => ({ ...prev, editpassword: value }))} />
      <InputPassword title="새 비밀번호 확인" value={passwordInfo.correctEditpassword} setEditPassword={(value) => setPasswordInfo((prev) => ({ ...prev, correctEditpassword: value }))} />
      {isShowingEditPWError && <span className={styles.validateMessage}>새 비밀번호가 일치하지 않아요.</span>}
