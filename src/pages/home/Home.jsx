...
function Home() {
  ...
  const mapedNotices = notices.map(({ id, title, textContent, tags, author, createdTime }) => {
  ...
  const mapedCurriculums = curriculums.map((curriculum) => {
    ...
    return (
      <CurriculumPreview key={curriculum.curriculum_id} subject={curriculum.subject} time={`${curriculum.time}h`} startDate={curriculum.startDate} endDate={curriculum.endDate}></CurriculumPreview>
    );
  });

  return (
    <div className={styles.wrapper}>
      <Header headerText="병가ㆍ공가ㆍ조퇴를 신청해야하나요?" onButtonAction={handleAttendance} />
